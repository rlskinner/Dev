...
double orgomega1 = 0.48566516831488;
double orgomega2 = 0.90519373301868;
#define CONSC	(0.001)
  ...
  while (1) {
	...
	switch (qread (&val)) {
	  ...
	  case LEFTMOUSE:  /* pick new omega (the translation factor) */
	   ...
	   /* omega1 = orgomega1 + CONSC*((-WINSIZE/2 + mval[0] - xorg) / (float) WINSIZE);
	    omega2 = orgomega2 + CONSC*((-WINSIZE/2 + mval[1] - yorg) / (float) WINSIZE);
*/
	    printf ("omega = %f %f\to1:o2 = %f\n", omega1, omega2, omega1/omega2);
	    theta = psi = 0;
            computeD( buf );
	    reset_everything();
	    break;
